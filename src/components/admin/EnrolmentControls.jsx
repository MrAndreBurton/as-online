...
export default function EnrolmentControls({
  ...
  const availableOfferings = offerings.filter(
    (item) => !liveOfferingIds.has(item.offering_id)
  );
  ...
  async function changeStatus(enrolmentId, status) {
    setWorking(true);
    setError("");
    try {
      await updateEnrolmentStatus(enrolmentId, status);
      await onChanged?.();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <section className="portal-card">
      <div className="aeos-section-heading">
        <div>
          <p className="portal-eyebrow">AEOS Offerings</p>
          <h3>Enrolments</h3>
        </div>
      </div>
            ...
            <tbody>
              {enrolments.map((item) => (
                <tr key={item.enrolment_id}>
                  <td>{item.offering?.offering_name || item.offering_id}</td>
                  <td>{item.offering?.subject?.subject_name || "—"}</td>
                  <td>
                  ...
                  <td>
                    <select
                      ...
                      disabled={working}
                      onChange={(e) =>
                        changeStatus(item.enrolment_id, e.target.value)
                      }
                    >
